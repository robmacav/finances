...
function formatDateDDMMYYYY(date: Date | isValidDate): string {
  if (!date) return "";
...
export function NewDialog({ onTransactionCreated }: NewProps) {
  const { data: formData } = useFormData();
    ...
    >
      ...
      <DialogContent className="md:min-w-3xl xl:min-w-6xl h-[90vh] flex flex-col">
        ...
        <Tabs value={tab} onValueChange={setTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:my-5 gap-4">
